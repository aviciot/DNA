...
def _build_notification_html(sections: dict, portal_url: str = "", iso_code: str = "", iso_name: str = "", language: str = "en") -> str:
    """Render LLM-generated sections into a branded HTML email with optional portal CTA button."""
    is_rtl = language == "he"
    dir_attr   = "rtl" if is_rtl else "ltr"
    text_align = "right" if is_rtl else "left"
    # Accent border flips side in RTL
    accent_border = "border-right:4px solid #2563eb;border-radius:8px 0 0 8px" if is_rtl else "border-left:4px solid #2563eb;border-radius:0 8px 8px 0"

    # Section labels — translated for Hebrew
    _LABELLED_EN = {
        ...
        "reminder_intro":   "Annual Review",
        "evidence_summary": "Evidence Required",
        "action_guidance":  "What To Do",
        "what_it_means":    "What This Means",
        "what_is_preserved":"Your Data",
        "what_to_expect_2": "What To Expect",
    }
    _LABELLED_HE = {
        "iso_overview":     "על תקן ה-ISO שלך",
        "journey_overview": "מסע ההסמכה שלך",
        "email_channel":    "כיצד עובדת איסוף הראיות",
        "portal_intro":     "פורטל הציות שלך",
        "what_to_expect":   "מה קורה עכשיו",
        "next_steps":       "צעדים הבאים",
        "reminder_intro":   "סקירה שנתית",
        "evidence_summary": "ראיות נדרשות",
        "action_guidance":  "מה לעשות",
        "what_it_means":    "מה זה אומר",
        "what_is_preserved":"הנתונים שלך",
        "what_to_expect_2": "מה לצפות",
    }
    _LABELLED = _LABELLED_HE if is_rtl else _LABELLED_EN

    _portal_btn_label = "🔗 פתח את פורטל הציות שלך" if is_rtl else "🔗 Open Your Compliance Portal"
    _portal_link_label = "או העתק קישור זה:" if is_rtl else "Or copy this link:"
    _footer_text = "הודעה זו נשלחה אוטומטית על ידי פלטפורמת DNA Compliance. לשאלות, השב למייל זה וצוות הציות שלנו ישמח לסייע." if is_rtl else "This message was sent automatically by DNA Compliance Platform. If you have questions, reply to this email and our compliance team will assist you."

    # Keys rendered as plain paragraphs only
    _PLAIN = {"greeting", "intro", "portal_section", "closing", "body"}
    _SKIP = {"subject"}

    greeting = sections.get("greeting", "שלום," if is_rtl else "Hello,")
    ...
    # intro / body always first plain paragraph
    for intro_key in ("intro", "body"):
        intro = sections.get(intro_key, "")
        if intro:
    ...
    for key, label in _LABELLED.items():
        ...
        if key == "portal_intro" and portal_url:
            ...
            if not val:
                continue
        parts.append(f"""
        <div style="margin:20px 0;padding:16px 20px;background:#f8fafc;{accent_border}">
    ...
    # Remaining unhandled sections
    handled = set(_LABELLED) | _PLAIN | _SKIP | {"intro", "body"}
    ...
    if portal_url:
      ...
      <div style="text-align:center;margin:32px 0 8px">
        ...
        <p style="margin:10px 0 0;font-size:12px;color:#94a3b8">
          {_portal_link_label} <a href="{portal_url}" style="color:#2563eb;word-break:break-all">{portal_url}</a>
        </p>
      </div>"""

    inner = "\n".join(parts)
    return f"""<!DOCTYPE html>
<html lang="{language}" dir="{dir_attr}"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="margin:0;padding:0;background:#f1f5f9;font-family:'Inter',Helvetica,Arial,sans-serif;direction:{dir_attr};text-align:{text_align}">
<table width="100%" cellpadding="0" cellspacing="0" role="presentation">
  <tr><td align="center" style="padding:40px 16px">
    <table width="600" cellpadding="0" cellspacing="0" role="presentation"
      ...
      <tr><td style="background:linear-gradient(135deg,#1d4ed8 0%,#2563eb 60%,#3b82f6 100%);padding:28px 36px">
        <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
          <tr>
            ...
            <td align="{('left' if is_rtl else 'right')}">
      ...
      <!-- Body -->
      <tr><td style="padding:36px 36px 20px;direction:{dir_attr};text-align:{text_align}">
        {inner}
        {portal_btn}
      </td></tr>

      <!-- Footer -->
      <tr><td style="background:#f8fafc;padding:20px 36px;border-top:1px solid #e2e8f0;direction:{dir_attr};text-align:{text_align}">
...
async def send_notification_email(
    ...
    html = _build_notification_html(sections, portal_url=portal_url, iso_code=iso_code, iso_name=iso_name, language=language)
