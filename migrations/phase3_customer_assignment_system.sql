-- =====================================================
-- Phase 3: Customer Assignment System - Migration
-- Date: 2026-02-13
...
CREATE INDEX idx_customer_config_customer ON dna_app.customer_configuration(customer_id);
CREATE INDEX idx_customer_config_type ON dna_app.customer_configuration(config_type);
CREATE INDEX idx_customer_config_active ON dna_app.customer_configuration(is_active);
...
-- =====================================================
-- 2. ALTER: tasks TABLE (Add Manual Tasks & Ignored Status)
-- =====================================================

-- Add new columns to tasks table
ALTER TABLE dna_app.tasks
...
ADD COLUMN IF NOT EXISTS ignored_by INTEGER REFERENCES auth.users(id),
ADD COLUMN IF NOT EXISTS ignore_reason TEXT,
ADD COLUMN IF NOT EXISTS created_manually_by INTEGER REFERENCES auth.users(id),
ADD COLUMN IF NOT EXISTS manual_task_context TEXT;

COMMENT ON COLUMN dna_app.tasks.is_auto_generated IS 'true = system generated from questions, false = manually created by admin';
COMMENT ON COLUMN dna_app.tasks.is_ignored IS 'true = task marked as irrelevant/ignored (e.g. template removed)';
COMMENT ON COLUMN dna_app.tasks.created_manually_by IS 'User who manually created this task (NULL if auto-generated)';
COMMENT ON COLUMN dna_app.tasks.manual_task_context IS 'Additional context for manual tasks';

-- =====================================================
-- 3. ALTER: customer_template_assignments TABLE (Add Ignored Status)
-- =====================================================

ALTER TABLE dna_app.customer_template_assignments
ADD COLUMN IF NOT EXISTS is_ignored BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS ignored_at TIMESTAMP,
ADD COLUMN IF NOT EXISTS ignored_by INTEGER REFERENCES auth.users(id),
ADD COLUMN IF NOT EXISTS ignore_reason TEXT;

COMMENT ON COLUMN dna_app.customer_template_assignments.is_ignored IS 'true = assignment marked as irrelevant (but kept for history)';
...
CREATE TABLE IF NOT EXISTS dna_app.task_resolutions (
    ...
    task_id UUID NOT NULL REFERENCES dna_app.tasks(id) ON DELETE CASCADE,
    ...
    follow_up_task_id UUID REFERENCES dna_app.tasks(id),
...
CREATE INDEX idx_task_resolution_task ON dna_app.task_resolutions(task_id);
CREATE INDEX idx_task_resolution_type ON dna_app.task_resolutions(resolution_type);
CREATE INDEX idx_task_resolution_requires_approval ON dna_app.task_resolutions(requires_approval) WHERE requires_approval = true;
...
CREATE TABLE IF NOT EXISTS dna_app.task_templates (
    ...
    task_level VARCHAR(50) NOT NULL, -- customer, assignment, document, question, iso
...
CREATE INDEX idx_task_template_type ON dna_app.task_templates(task_type);
CREATE INDEX idx_task_template_active ON dna_app.task_templates(is_active);
CREATE INDEX idx_task_template_level ON dna_app.task_templates(task_level);
...
COMMENT ON COLUMN dna_app.iso_standards.code IS 'ISO standard code (use "stand_alone" for non-ISO templates)';
...
SELECT
    ...
    -- Template counts
    COUNT(DISTINCT cta.id) as total_templates,
    COUNT(DISTINCT CASE WHEN cta.status = 'completed' THEN cta.id END) as completed_templates,
    COUNT(DISTINCT CASE WHEN cta.status IN ('in_progress', 'draft_saved') THEN cta.id END) as in_progress_templates,

    -- Task counts (excluding ignored)
    COUNT(DISTINCT t.id) FILTER (WHERE t.is_ignored = false) as total_tasks,
    COUNT(DISTINCT t.id) FILTER (WHERE t.status = 'completed' AND t.is_ignored = false) as completed_tasks,
    COUNT(DISTINCT t.id) FILTER (WHERE t.status = 'in_progress' AND t.is_ignored = false) as in_progress_tasks,
    COUNT(DISTINCT t.id) FILTER (WHERE t.status = 'pending' AND t.is_ignored = false) as pending_tasks,
    COUNT(DISTINCT t.id) FILTER (WHERE t.is_ignored = true) as ignored_tasks,

    -- Progress percentage (excluding ignored tasks)
    CASE
        WHEN COUNT(DISTINCT t.id) FILTER (WHERE t.is_ignored = false) = 0 THEN 0
        ELSE (COUNT(DISTINCT t.id) FILTER (WHERE t.status = 'completed' AND t.is_ignored = false)::float /
              COUNT(DISTINCT t.id) FILTER (WHERE t.is_ignored = false)::float * 100)::integer
    END as progress_percentage,

    -- Target dates
    cis.target_certification_date,
...
FROM dna_app.customer_iso_standards cis
LEFT JOIN dna_app.iso_standards iso ON cis.iso_standard_id = iso.id
LEFT JOIN dna_app.customer_template_assignments cta
    ON cis.customer_id = cta.customer_id
    AND cis.iso_standard_id = cta.iso_standard_id
    AND cta.is_ignored = false
LEFT JOIN dna_app.tasks t
    ON cta.id = t.assignment_id
GROUP BY cis.id, cis.customer_id, cis.iso_standard_id, iso.code, iso.name,
         cis.target_certification_date, cis.certification_status, cis.created_at, cis.updated_at;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON dna_app.customer_configuration TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON dna_app.task_resolutions TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON dna_app.task_templates TO authenticated;

-- Grant permissions on view
GRANT SELECT ON dna_app.v_customer_iso_progress TO authenticated;
...
-- Verify columns added to customer_template_assignments
SELECT column_name
FROM information_schema.columns
WHERE table_schema = 'dna_app'
AND table_name = 'customer_template_assignments'
AND column_name IN (
    'is_ignored',
    'ignored_at',
    'ignored_by',
...
SELECT template_name, task_type, task_level
